...
if (isset($_GET["action"])) {
    ...
    if ($a === "add") {
                ...
                if (isset($_POST["publish"])) {
                    ...
                    if (empty($name) || empty($credits) || empty($hours)) {
                        ...
                        try {
                            ...
                            $args = array($name, $body, $credits, $hours, $semester, $department);
        ...
        if (isset($_GET["id"])) {
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                        ...
                        if (empty($name) || empty($credits) || empty($hours)) {
                            ...
                            try {
                                $stmt = "UPDATE courses SET course_name = ?,"
                                        ...
                                        . " course_id = ?";
                                $args = array($name, $body, $credits, $hours, $semester, $department, $id);
                                $db->runQuery($stmt, $args);
                    ...
                    $db = new Database();
                    $db->query("SELECT * FROM courses WHERE course_id = ?", array($id));
                    $r = $db->fetchObject();
                    ?>
                    <form method="post" action="<?php echo $_SERVER["REQUEST_URI"] ?>">
                        ...
                        <div class="form-group">
                            <label>Course Body</label>
                            <textarea class="form-control editor" name="body" rows="10"><?php echo $r->course_body ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>Course Credit</label>
                            <input type="number" name="credit" value='<?php echo $r->course_credit ?>' class="form-control" required="" />
                        </div>
                        <div class="form-group">
                            <label>Course Hours</label>
                            <input type="number" name="hours" value='<?php echo $r->course_hours ?>' class="form-control" required="" />
                        </div>
                        <div class="form-group">
                            <label>Course Semester</label>
                            <select name="semester" class="form-control">
        ...
        if (isset($_GET["id"])) {
            $id = intval(cleanString($_GET["id"]));
            try {
                $db->runQuery("DELETE FROM courses WHERE course_id = ?", array($id));
    ...
    <div class="panel panel-primary">
        <div class="panel-heading">
            <div class="panel-title">
                Manage Courses
                <div class="pull-right">
                    <a href="courses.php?action=add" class="btn btn-sm btn-success">Add Course</a>
                </div>
                <div class="clearfix"></div>
            </div>
            ...
            <table class="table table-striped table-hover">
                ...
                try {
                    ...
                    while ($r = $db->fetchObject()) {
                        $xdb = new Database();
                        $xdb->query("SELECT department_name FROM departments WHERE department_id = ?", array($r->course_department));
                        $department = $xdb->fetchObject()->department_name;
                        echo "
                            <tr>
                                <td>$r->course_name</td>
                                ...
                                <td>
                                    <div class='btn-group'>
                                        ...
                                        <a href='courses.php?action=edit&amp;id=$r->course_id' class='btn btn-primary'>Edit</a>
