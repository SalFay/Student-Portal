    ...
    if ($a === "add") {
        ...
        <div class="panel panel-primary">
            ...
            <div class="panel-body">
                ...
                if (isset($_POST["publish"])) {
                    ...
                    if (empty($title) || empty($body) || empty($expiry)) {
                        ...
                        try {
                            ...
                            $students = [];
                            $req = $db->queryValues("SELECT course_department, course_semester FROM courses WHERE course_id = ?", ["course_department", "course_semester"], $course);
                            $db->query("SELECT * FROM admission WHERE department_id = ? AND semester_id = ?", [$req->course_department, $req->course_semester]);
                            $db2 = new Database();
                ...
                ?>
                <form method="post" action="assignments.php?action=add">
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <table class="tabel table-hover table-striped">
                        ...
                        while ($r = $db->fetchObject()) {
                            $student = $db->queryValues("SELECT * FROM users WHERE user_id = ?", "user_name", $r->student_id);
                    ...
                    <tr>
                        ...
                        <td>$r->af_comments</td>
                        <td>$r->af_marks</td>
                        <td><a href='assignments.php?do=edit-file&amp;id=$r->af_id' class='btn btn-primary'>Edit</a></td>
                    </tr>
                ";
                        }
                        ?>
    ...
    <div class="panel-body">
        ...
        <form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
            <div class="form-group">
                ...
                <textarea class="form-control" name="comments"><?php echo $r->af_comments ?></textarea>
            </div>
            <div class="form-group">
                <label>Marks</label>
                <input type="text" name="marks" value="<?php echo $r->af_marks ?>" class="form-control" />
            </div>
...
<?php
        ...
        if (isset($_GET["id"])) {
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <form method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>">
                        <div class="form-group">
                            <label>Title</label>
                            <input type="text" name="title" value="<?php echo $r->asgn_title ?>" class="form-control" required="" />
                        </div>
                        <div class="form-group">
                            <label>Details</label>
                            <textarea class="form-control" name="body" rows="10"><?php echo $r->asgn_title ?></textarea>
                        </div>
                        <div class="form-group">
                            <label>Expiry Date</label>
                            <input type="date" name="expiry" class="form-control" required="" value="<?php echo date("m/d/Y", $r->asgn_expiry) ?>" />
                        </div>
                        <div class="form-group">
                            <label>Status</label>
                            <select name="status" class="form-control">
            ...
            <?php echo list_courses($r->course_id); ?>
                        ...
                        <input type="submit" name="publish" value="Update Assignment" class="btn btn-primary" />
                    </form>
                </div>
            </div>
            <?php
        ...
        if (isset($_GET["id"])) {
            $id = intval(cleanString($_GET["id"]));
            try {
                $db->runQuery("DELETE FROM assignments WHERE asgn_id = ?", array($id));
                $db->runQuery("DELETE FROM assignment_files WHERE asgn_id = ?", array($id));
        ...
        <div class="panel-heading">
            <div class="panel-title">
                ...
                <div class="pull-right">
                    <a href="assignments.php?action=add" class="btn btn-sm btn-success">Add Assignment</a>
                </div>
            ...
            <table class="table table-striped table-hover">
                ...
                try {
                    if ($_SESSION["user_level"] == "admin") {
                        $db->query("SELECT * FROM assignments ORDER BY asgn_id DESC");
                    } else {
                        $db->query("SELECT * FROM assignments WHERE teacher_id = ? ORDER BY asgn_id DESC", $user_id);
                    }
                    while ($r = $db->fetchObject()) {
                        $course = $db->queryValues("SELECT * FROM courses WHERE course_id = ?", "course_name", $r->course_id)->course_name;
                        ...
                        echo "
                            <tr>
                                ...
                                <td>
                                    <div class='btn-group'>
                                        <a href='assignments.php?action=view&amp;id=$r->asgn_id' class='btn btn-success'>View</a>
                                        <a href='assignments.php?action=edit&amp;id=$r->asgn_id' class='btn btn-primary'>Edit</a>
                                        <a href='assignments.php?action=delete&amp;id=$r->asgn_id' class='btn btn-danger'>Delete</a>
                                    </div>
                                </td>
                            </tr>
                        ";
